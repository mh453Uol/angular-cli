...
export default Task.extend({
  run: function(options: UpdateTaskOptions) {
      ...
      return newVersionAvailable(rootDir, packageName, options)
        .then((result: CheckResult) => {
          ...
          if (newVersionAvailable) {
            this.ui.writeLine(chalk.yellow(oneLine`Current version (${result.currentVersion}) is out of date
              consider updating to the new version (${result.newVersion})`));
          }
